...
--
DELIMITER $$
CREATE TRIGGER `trg_delete_libro` AFTER DELETE ON `tabla_libro` FOR EACH ROW BEGIN
...
NOW(),
USER(),
'DELETE EN TABLA_LIBRO'
);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_insert_libro` AFTER INSERT ON `tabla_libro` FOR EACH ROW BEGIN
INSERT INTO audi_socio (
AUDI_FECHA_MODIFICACION,
AUDI_USUARIO,
AUDI_ACCION
...
NOW(),
USER(),
'INSERT EN TABLA_LIBRO'
);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_update_libro` AFTER UPDATE ON `tabla_libro` FOR EACH ROW BEGIN
INSERT INTO audi_socio (
AUDI_FECHA_MODIFICACION,
AUDI_USUARIO,
AUDI_ACCION
